    ...
    upload.single('file')(req, res, async (err) => {
      ...
      try {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          const requiredFields = ['name', 'number', 'address', 'totalCapacity', 'shiftACapacity', 'shiftBCapacity', 'shiftCCapacity'];
          ...
          const numericFields = ['totalCapacity', 'shiftACapacity', 'shiftBCapacity', 'shiftCCapacity'];
          for (const field of numericFields) {
            const value = parseInt(stationData[field]);
          ...
          const existingStation = await Station.findOne({ number: stationData.number });

          if (existingStation) {
            results.errors.push({
              row: i + 1,
              error: `Station already exists with number ${stationData.number}`
            });
            results.skipped++;
            continue;
          }
          ...
          try {
            ...
            const station = new Station({
              ...
              totalCapacity: stationData.totalCapacity,
              shiftCapacity: {
                A: stationData.shiftACapacity,
                B: stationData.shiftBCapacity,
                C: stationData.shiftCCapacity
              },
              currentAssignments: [],
              availablePositions: {
                A: stationData.shiftACapacity,
